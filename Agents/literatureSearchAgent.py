...
def literature_search_agent():
    """
    Initializes and returns a configured instance of the Scholar Research Agent.

    This agent is designed to assist with academic literature discovery and synthesis.
    It is equipped with tools such as arXiv for accessing scholarly papers and DuckDuckGo
    for supporting web search, along with reasoning tools for advanced query handling.

    The agent is instructed to return a detailed, scholarly markdown summary of research
    findings, formatted in a structured and academic tone.

    Returns:
        Agent: An instance of the configured Scholar Research Agent.
    """
    agent = Agent(
...
def run_literature_search(query: str):
    """
    Executes a literature search using the Scholar Research Agent for a given query.

    The function initializes the agent, runs the query to retrieve academic insights,
    and saves the result to a vector database for future reference or retrieval.

    Args:
        query (str): The user's input topic or research question.

    Returns:
        str: The scholarly markdown-formatted response generated by the agent.
    """
